...
Select p.PRO_ID,p.PRO_NAME from `order` o inner join supplier_pricing sp on o.PRICING_ID=sp.PRICING_ID inner join product p on sp.PRO_ID=p.PRO_ID where ORD_DATE>'2021-10-05';

#8)	Display customer name and gender whose names start or end with character 'A'.
Select CUS_NAME,CASE WHEN CUS_GENDER='M' Then 'Male' When CUS_GENDER='F' then 'Female' END GENDER from CUSTOMER where CUS_Name like 'A%' or CUS_Name like '%A';
...
on test.pricing_id = supplier_pricing.pricing_id) as test2 group by test2.supp_id) 
